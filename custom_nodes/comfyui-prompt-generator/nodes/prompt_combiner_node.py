...
class PromptCombinerNode:
    ...
    def combine(
        ...
        if selected == CombineMode.BLEND:
            return (self._blend(prompts),)
        elif selected == CombineMode.CONCAT:
            return (self._concat(prompts, separator),)
        elif selected == CombineMode.WEIGHTED_AVERAGE:
            return (self._weighted_average(prompts),)

        # Fallback — should never reach here
        return ("[PromptCombiner] Internal error: unhandled mode.",)
